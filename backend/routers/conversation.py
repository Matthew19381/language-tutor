...
@router.get("/api/v1/voice-chat/prompt")
async def get_voice_chat_prompt(user_id: int, db: Session = Depends(get_db)):
    user = db.query(User).filter(User.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    ...
    vocab_section = "\nSĹ‚ownictwo z dzisiejszej lekcji: " + ", ".join(vocab_list) if vocab_list else ""
    prompt = f"""JesteĹ› nauczycielem jÄ™zyka {user.target_language}. MĂłj jÄ™zyk ojczysty to polski.
MĂłj aktualny poziom: {user.cefr_level}
Dzisiaj uczyĹ‚em/am siÄ™: {topic} ({lesson_title}){vocab_section}
Moje sĹ‚abe obszary: {weak_areas_text}

ProszÄ™, rozmawiaj ze mnÄ… po {user.target_language} na poziomie {user.cefr_level}. Poprawiaj moje bĹ‚Ä™dy delikatnie (pokazuj poprawnÄ… formÄ™ w nawiasach kwadratowych). Postaraj siÄ™ uĹĽyÄ‡ w rozmowie sĹ‚ownictwa z dzisiejszej lekcji. PomĂłĹĽ mi Ä‡wiczyÄ‡: {topic}. Zacznij od krĂłtkiego wprowadzenia do dzisiejszego tematu, a nastÄ™pnie zaangaĹĽuj mnie w rozmowÄ™."""
