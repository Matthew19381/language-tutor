"""
Grok Router — konwersacja przez OpenRouter + generator promptów.
...
@router.get("/api/v1/grok/prompt/{user_id}")
def generate_grok_prompt(user_id: int, db: Session = Depends(get_db)):
    """
    Generuj prompt dla Grok/OpenRouter na podstawie dzisiejszej aktywności użytkownika.
    Prompt zawiera: lekcję, błędy, słownictwo, plan nauki.
    """
...
@router.post("/api/v1/grok/conversation/voice")
async def grok_voice_conversation(request: dict):
    """
...
@router.post("/api/v1/grok/conversation/text")
async def grok_text_conversation(request: dict):
    """
